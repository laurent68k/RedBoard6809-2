;
;	Coco Basic Microsoft (Color Basic 1982)
;   original file from Grant's 6-chip 6809 computer : http://searle.x10host.com/6809/Simple6809.html
;	
;	Version:	0.0
;	Created:	2025 July 23
;	Updated:	
...
;   FAVARD Laurent
;
;	Updated to work on RedBoard-2 6809 inside an EEPROM of 16kB
;	----------------------------------------------------------------------------------------------------                               
...
* ANY ADDITIONAL TABLES WILL HAVE TO BE                      
...
INITACIA  LDA  #21            DIV16 CLOCK -> 7372800 / 4 / 16 = 115200 
          STA  UCTRL           
...
LA084:    LDA  128,X            LOOK FOR END
          COMA                * COMPLEMENT IT AND PUT IT BACK
          STA  128,X            * INTO SYSTEM MEMORY
          CMPA 128,X            IS IT RAM?
          BNE  LA093          BRANCH IF NOT (ROM, BAD RAM OR NO RAM)
          LEAX 127,X            MOVE POINTER UP ONE
          COM  127,X            RE-COMPLEMENT TO RESTORE BYTE
          BRA  LA084          KEEP LOOKING FOR END OF RAM 
		  
LA093:    ;ldy	#$7FFF			;	LFD: top of RedBoard RAM
          ...
          LDU  #LPTCFW        POINT U TO RAM DESTINATION (WAS #CMPMID) 
          LDB  #18            MOVE 18 BYTES (WAS 28) 
          JSR  LA59A          MOVE 18 BYTES FROM ROM TO RAM 
          LDU  #IRQVEC        POINT U TO NEXT RAM DESTINATION 
          LDB  #24            MOVE 24 MORE BYTES (WAS 30) 
...
LA0B6     STX  3,U            * SET EXBAS COMMAND INTERPRETATION 
          ...
          LDX  #RVEC2         POINT X TO START OF RAM VECTORS 
          LDD  #$392A         SET UP TO SAVE 42 (WAS 75) RTS� 
...
LA1B5     BSR  KEYIN          GO CHECK KEYBOARD 
          BEQ  LA1B5          LOOP IF NO KEY DOWN 
LA1BF     RTS                  
*                              
...
LB290     JSR  GETNCH         GET AN INPUT CHARACTER (SECONDARY TOKEN) 
          ...
          CMPB #2*28          29 SECONDARY FUNCTIONS - 1 
...
LB29F     PSHS B              SAVE TOKEN OFFSET ON STACK 
          CMPB #2*21          CHECK FOR NUMERIC ARGUMENT TOKEN 
          BCS  LB2C7          DO SECONDARIES HEX$ OR LESS 
          CMPB #2*24          * 
...
LB86B     CMPA #'?'            CHECK FOR "?" - PRINT ABBREVIATION 
          ...
          BRA  LB852          * IN BUFFER 
LB873     CMPA #$60            APOSTROPHE IS SAME AS REM 
...
L866B     LDA  ,X             Error 2029 
          BEQ  L8685          Error 2029 
...
L9015     LDA  ,-X            Error 2029 
          STA  $01,X          Error 2029 
          CMPX #STRBUF+3      Error 2029 
          BNE  L9015          Error 2029 
          LDA  #'%'           * INSERT A % SIGN AT START OF 
          STA  ,X             * STRING - OVERFLOW ERROR 
          RTS                  
                               
L9023     LDA  FP0EXP         Error 2029 
          STA  V47            Error 2029 
...
;	----------------------------------------------------------------------------------------------------
;	LFD: Code in ROM, CHARAD stay in RAM because receive a pointer
							   
GETNCH    INC  <CHARAD+1      *PV INCREMENT LS BYTE OF INPUT POINTER 
          BNE  GETCCH         *PV BRANCH IF NOT ZERO (NO CARRY) 
          INC  <CHARAD        *PV INCREMENT MS BYTE OF INPUT POINTER 
GETCCH    lda	[CHARAD]           *PV OP CODE OF LDA EXTENDED 
